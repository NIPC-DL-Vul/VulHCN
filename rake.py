...
def load_stop_words(stop_word_file):
    ...
    从文件中加载止损词并以单词列表的形式返回的实用函数
    @param stop_word_file 包含停止词的文件的路径和文件名。
    @return list 一个停止词的列表。
    """
    stop_words = []
...
def separate_words(text, min_word_return_size):
    ...
    实用函数，返回一个长度大于指定字符数的所有单词的列表。
    @param text 必须被分割成单词的文本。
    @param min_word_return_size 一个单词必须包含的最小字符数。
    """
...
# return adjoined_candidates 相邻候选人
...
def adjoined_candidates_from_sentence(s, stoplist, min_keywords, max_keywords):
    ...
    # 初始化候选人列表
    candidates = []
    # Splits the sentence to get a list of lowercase words 小写字母
    sl = s.lower().split()
...
class Rake(object):
    def __init__(self, stop_words_path, min_char_length=1, max_words_length=1, min_keyword_frequency=1,
                 min_words_length_adj=1, max_words_length_adj=1, min_phrase_freq_adj=2):
    ...
    def run(self, text, min_keyword_score):
        ...
        a = ''
        b = 0
        print("=======================>each_keypair is :")
        for each_keypair in sorted_keywords:
            # print(each_keypair)
            if each_keypair[1] > min_keyword_score:
                output_keywords.append(each_keypair[0])
                a = a + each_keypair[0] + ';'
                # print(each_keypair[1])
                b = b + 1
...
if test and __name__ == '__main__':
    ...
    stop_path = "data/stoplists/SmartStoplist.txt"
    '''
